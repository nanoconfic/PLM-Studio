...
if ($startNew) {
    ...
    $history+=@([ordered]@{
        ...
        validation=$c.validation;knowledge_capture=$c.knowledge_capture;result=$c.result;final_status=$c.status
    })
    ...
    $c.workflow=[ordered]@{current_iteration=$next;phase='Requirements';requirements_confirmed=$false;next_action='Collect iteration requirements'}
...
Write-Output 'Ask only for missing items: change goal; current problem; user scenario; inputs; UI/interaction; acceptance criteria; constraints; out of scope.'
Write-Output 'Loading applicable knowledge into the current context. Rerun with precise facets after requirements are confirmed.'
