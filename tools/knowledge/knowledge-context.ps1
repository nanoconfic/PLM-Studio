...
elseif ($selected -contains 'Auto') {
    $capability=[string]$extensionConfig.workflow.capability_mode
    $stage=[string]$extensionConfig.workflow.stage
    if ($capability -in @('integration','linked') -and $stage -eq 'integration') {
        $selected=@('Integration','Environment','Business')
        if ($extensionConfig.delivery.mode -eq 'backend') { $selected+='Data' }
    } elseif ($capability -in @('prototype','linked')) {
        $selected=@('Style','Business')
        if ($extensionConfig.delivery.demo_data -notin @('','none','pending') -or $extensionConfig.delivery.mode -eq 'backend') { $selected+='Data' }
    } else {
        $selected=@('Style','Business')
