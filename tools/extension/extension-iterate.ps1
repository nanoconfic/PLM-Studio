...
if ($startNew) {
    Assert-ControllerAction $w $c 'start_iteration' | Out-Null
    ...
    $history+=@([ordered]@{
        ...
        integration_requirements=$c.integration_requirements;integration_evidence=$c.integration_evidence;workflow=$c.workflow
    ...
    $c.integration_requirements=New-IntegrationRequirementState
    $c.integration_evidence=New-IntegrationEvidenceState
    $c.validation=New-ValidationState
    $c.knowledge_capture=New-KnowledgeCaptureState
    $c.style_context=New-StyleContextState
    $c.result=[ordered]@{summary=$null;artifacts=@();prototype_review=$null;completed_at=$null}
    $mode=[string]$c.workflow.capability_mode
    $stage=$(if ($mode -eq 'integration') {'integration'} else {'prototype'})
    $c.workflow=[ordered]@{
        current_iteration=$next;capability_mode=$mode;stage=$stage;prototype_accepted=$false;phase='Requirements'
        delivery_confirmed=([string]$c.delivery.mode -in @('static','static-demo','embedded-static','backend'))
        requirements_confirmed=$false;next_action=$(if ($stage -eq 'integration') {'Collect and confirm integration requirements'} else {'Collect and confirm prototype design requirements'})
    }
...
Write-Output "Ask only for missing items in the $($c.workflow.stage) stage. Query controller/run.ps1 for the current checklist."
Write-Output 'Do not implement until the matching preflight passes.'
